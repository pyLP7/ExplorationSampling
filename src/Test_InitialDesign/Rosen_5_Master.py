...
def Ackley(x):
    d = len(x)
    sum_sq_term = -20 * np.exp(-0.2 * np.sqrt(sum(np.array(x)*np.array(x)) / d))
    cos_term = -np.exp(sum(np.cos(2*np.pi*np.array(x)) / d))
    return (20 + np.exp(1) + sum_sq_term + cos_term) #10e6*

def Rastrigin(x):
    value=0
    for i in range(len(x)):
        value=value+x[i]**2 - 10 * np.cos(2 * np.pi * x[i])
    return (value+10*len(x))
...
inputParameters = ['t_'+str(ii+1) for ii in range(5)] 
...
at=os.getcwd()
file=open(path1,"w")
file.write(format('ObjectiveFunction f(x)','<75') + '=' +  format(str(Ackley(x)), '>40')+'\n')
file.write('\n')
